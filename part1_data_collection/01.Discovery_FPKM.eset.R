#!/usr/bin/env Rscript
##prepare discovery expression eset
...
Discovery_FPKM.eset<-eset

exprs(eset)<-log2(exprs(eset)+0.1)
Discovery_log2FPKM.eset<-eset
#####3.QC#####
draw.eset.QC(Discovery_eset, outdir = "./QC", do.logtransform = FALSE, prefix = 'Discovery_FPKM_',
...
draw.eset.QC(Discovery_log2FPKM.eset, outdir = "./QC", do.logtransform = FALSE, prefix = 'Discovery_log2FPKM_',
             intgroup = "condition", choose_plot = c("heatmap", "pca", "density", "correlation", "meansd"), generate_html = TRUE, correlation_strategy = "pearson", plot_all_point = FALSE,
             emb_plot_type='2D.text' # "2D", "2D.interactive", "2D.ellipse", "2D.text" or "3D" 
)
#save data
save(Discovery_FPKM.eset,Discovery_log2FPKM.eset,file = "./Discovery_esets")
